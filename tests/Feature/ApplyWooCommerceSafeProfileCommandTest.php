...
class ApplyWooCommerceSafeProfileCommandTest extends TestCase
{
    ...
    public function test_it_can_apply_safe_profile_to_single_woocommerce_store(): void
    {
        $store = $this->createWooStore('WOO-CMD-ONE');

        IntegrationSyncProfile::query()->create([
            'store_id' => $store->id,
            'orders_poll_minutes' => 10,
            'finance_poll_minutes' => 10,
        ...
        $this->artisan('marketplace:apply-woo-safe-profile', [
            '--store' => $store->id,
        ])->assertExitCode(0);
        ...
        $this->assertSame(
            IntegrationSyncProfile::recommendedWooWebhookTopics(),
        ...
        $this->assertSame(15, $profile->request_jitter_seconds);
    }

    public function test_it_does_not_write_in_dry_run_mode(): void
    {
        $store = $this->createWooStore('WOO-CMD-DRY');

        IntegrationSyncProfile::query()->create([
            'store_id' => $store->id,
            'orders_poll_minutes' => 12,
            'finance_poll_minutes' => 25,
            'products_poll_minutes' => 90,
