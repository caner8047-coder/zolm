...
class ApplyHepsiburadaSafeProfileCommandTest extends TestCase
{
    ...
    public function test_it_can_apply_safe_profile_to_single_hepsiburada_store(): void
    {
        $store = $this->createStore('hepsiburada', 'HB-CMD-ONE');
        ...
        $this->artisan('marketplace:apply-hepsiburada-safe-profile', [
            '--store' => $store->id,
        ])->assertExitCode(0);
        ...
        $this->assertSame(120, $profile->finance_poll_minutes);
    ...
    protected function createStore(string $marketplace, string $prefix): MarketplaceStore
    {
        ...
        $entity = LegalEntity::query()->create([
            'user_id' => $user->id,
            'name' => 'Zem Hepsiburada Command Ltd.',
            'tax_number' => '4' . $suffix,
