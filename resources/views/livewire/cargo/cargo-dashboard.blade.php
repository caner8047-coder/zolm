...
<div class="space-y-4 lg:space-y-6 overflow-hidden" wire:key="dashboard-{{ $period }}">
    <div class="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4 lg:gap-6">
        <div class="max-w-3xl">
            <h2 class="text-xl lg:text-2xl font-bold text-slate-900">Genel görünüm</h2>
        ...
        <div class="flex flex-wrap gap-2">
            @foreach($periodOptions as $optionKey => $optionLabel)
                <button
                    ...
                    class="min-h-[44px] rounded-md px-4 py-2 text-sm font-medium transition {{ $period === $optionKey ? 'bg-indigo-600 text-white shadow-sm' : 'border border-slate-300 bg-white text-slate-700 hover:bg-slate-50' }}"
                >
                    {{ $optionLabel }}
                </button>
            @endforeach
    ...
    <x-zolm.section-card variant="orders" padding="p-4 lg:p-5">
        <div class="space-y-4">
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3 lg:gap-4">
                <div>
                    <h3 class="text-base lg:text-lg font-semibold text-slate-900">Operasyon filtreleri</h3>
            ...
            @if($period === 'custom')
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:gap-4">
                    <div>
                        ...
                        <input type="date" wire:model.live="customStartDate" class="mt-1 min-h-[44px] w-full rounded-md border border-slate-300 px-3 py-2 text-base sm:text-sm text-slate-900 focus:border-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-200">
                    </div>
                    <div>
                        <label class="block text-xs sm:text-sm font-medium text-slate-500">Bitiş</label>
                        <input type="date" wire:model.live="customEndDate" class="mt-1 min-h-[44px] w-full rounded-md border border-slate-300 px-3 py-2 text-base sm:text-sm text-slate-900 focus:border-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-200">
                    </div>
                </div>
            @endif
            ...
            <div x-data="{ showAdvanced: @js($showAdvancedFilters) }" class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 lg:gap-4">
                    ...
                    <div>
                        ...
                        <select wire:model.live="filterMarketplace" class="mt-1 min-h-[44px] w-full rounded-md border border-slate-300 px-3 py-2 text-base sm:text-sm text-slate-900 focus:border-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-200">
                    ...
                    <div class="flex items-end gap-2">
                        <button type="button" @click="showAdvanced = !showAdvanced" class="min-h-[44px] flex-1 rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
        ...
        >
            <div class="space-y-4">
                <div class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
                    <p class="text-sm font-semibold text-slate-900">Ana hata tipleri</p>
                    <div class="mt-3 space-y-2">
                        @forelse(array_slice($errorDistribution['details'], 0, 5) as $error)
                            <div class="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 py-3">
                                <div class="min-w-0">
                                    <p class="truncate text-sm font-medium text-slate-900">{{ $error['name'] }}</p>
                                    <p class="mt-1 text-xs text-slate-500">{{ $formatCount($error['count']) }} kayıt · %{{ number_format($error['percentage'], 1, ',', '.') }}</p>
                                </div>
                                <span class="shrink-0 text-sm font-semibold {{ ($errorTones[$error['key']] ?? 'default') === 'danger' ? 'text-rose-600' : (($errorTones[$error['key']] ?? 'default') === 'success' ? 'text-emerald-600' : 'text-slate-900') }}">
                                    {{ $formatMoney($error['tutar']) }}
                                </span>
                            </div>
                        @empty
                            <div class="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-6 text-center text-sm text-slate-500">
                ...
                <div class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
                    <p class="text-sm font-semibold text-slate-900">En çok tekrar eden ürünler</p>
                    <div class="mt-3 space-y-2">
                        @forelse(array_slice($topErrorProducts, 0, 5) as $product)
                            <div class="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 py-3">
                                <div class="min-w-0">
                                    <p class="truncate text-sm font-medium text-slate-900">{{ $product['urun_adi'] ?: 'İsimsiz ürün' }}</p>
                                    <p class="mt-1 text-xs text-slate-500">{{ $product['stok_kodu'] ?: 'Stok kodu yok' }} · {{ $formatCount($product['error_count']) }} hata</p>
                                </div>
                                <span class="shrink-0 text-sm font-semibold {{ $product['total_tutar_fark'] > 0 ? 'text-rose-600' : ($product['total_tutar_fark'] < 0 ? 'text-emerald-600' : 'text-slate-900') }}">
        ...
        >
            <div class="space-y-3">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-2">
                    <div class="min-w-0 rounded-lg border border-l-4 border-slate-200 border-l-rose-500 bg-white p-2.5 shadow-sm">
                        <div class="flex items-start gap-2">
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Bize karşı</p>
                                ...
                                <p class="mt-0.5 text-[11px] leading-4 text-slate-500">{{ $formatCount($costAnalysis['against_us_count']) }} kayıt</p>
                            </div>
                        </div>
                    </div>

                    <div class="min-w-0 rounded-lg border border-l-4 border-slate-200 border-l-green-500 bg-white p-2.5 shadow-sm">
                        <div class="flex items-start gap-2">
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Lehimize</p>
                                <p class="mt-1 text-sm font-semibold leading-tight tracking-tight text-green-600">{{ $formatMoney($costAnalysis['for_us_tutar']) }}</p>
                                <p class="mt-0.5 text-[11px] leading-4 text-slate-500">{{ $formatCount($costAnalysis['for_us_count']) }} kayıt</p>
                            </div>
                        </div>
                    </div>

                    <div class="min-w-0 rounded-lg border border-l-4 border-slate-200 {{ $netTone === 'danger' ? 'border-l-rose-500' : ($netTone === 'success' ? 'border-l-green-500' : 'border-l-slate-400') }} bg-white p-2.5 shadow-sm">
                        <div class="flex items-start gap-2">
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Net sonuç</p>
                                <p class="mt-1 text-sm font-semibold leading-tight tracking-tight {{ $netTone === 'danger' ? 'text-rose-600' : ($netTone === 'success' ? 'text-green-600' : 'text-slate-900') }}">{{ $formatSignedMoney($costAnalysis['net_cost']) }}</p>
                                <p class="mt-0.5 text-[11px] leading-4 text-slate-500">Toplam etki</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-lg border border-slate-200 bg-slate-50/60 p-3.5">
                    <p class="text-sm font-semibold text-slate-900">Kanal kırılımı</p>
                    <div class="mt-3 space-y-3">
                        <div class="space-y-2">
                            <p class="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Pazaryeri</p>
                            @forelse(array_slice($marketplaceInsights, 0, 3) as $insight)
                                <div class="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2.5">
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-medium text-slate-900">{{ $insight['label'] }}</p>
                                        <p class="mt-0.5 text-xs text-slate-500">{{ $formatCount($insight['errors']) }} hata / {{ $formatCount($insight['total']) }} kayıt</p>
                                    </div>
                                    <span class="shrink-0 text-sm font-semibold text-slate-900">{{ $formatMoney($insight['tutar_impact']) }}</span>
                                </div>
                            @empty
                                <div class="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-3.5 text-center text-sm text-slate-500">Veri yok.</div>
                            @endforelse
                        </div>

                        <div class="space-y-2">
                            <p class="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Mağaza</p>
                        ...
                        <div class="space-y-2">
                            <p class="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Kategori</p>
