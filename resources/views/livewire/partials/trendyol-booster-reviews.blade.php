...
>
    ...
    <div class="border-b border-slate-200 bg-white px-4 py-3 lg:px-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="inline-flex w-full rounded-[8px] border border-slate-200 bg-slate-50/60 p-1 sm:w-auto">
                <button type="button" wire:click="setReviewWorkspaceTab('reviews')"
                        class="min-h-[44px] flex-1 rounded-[6px] px-4 py-2 text-sm font-medium transition sm:flex-none {{ $reviewWorkspaceTab === 'reviews' ? 'bg-white text-slate-900 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:text-slate-800' }}">
                    Yorum Havuzu
                </button>
                <button type="button" wire:click="setReviewWorkspaceTab('insights')"
                        class="min-h-[44px] flex-1 rounded-[6px] px-4 py-2 text-sm font-medium transition sm:flex-none {{ $reviewWorkspaceTab === 'insights' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800' }}">
                    AI İçgörü
                </button>
            </div>
            <p class="text-xs text-slate-500">Aynı yorum kaynağı · ayrı veri kopyası oluşturulmaz</p>
        </div>
    </div>

    @if($reviewWorkspaceTab === 'reviews')
    {{-- Ürün Eşleme Paneli --}}
    @if($reviewStats && ($reviewStats['total'] ?? 0) > 0)
        @php
    ...
    @else
        @include('livewire.partials.trendyol-booster-review-insights')
    @endif
</section>
