<div class="space-y-6 overflow-x-hidden">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl lg:text-2xl font-bold text-gray-900">⚡ Flaş Ürünler</h1>
            <p class="text-sm lg:text-base text-gray-500 mt-1">Flaş kampanya fiyatlarının kârlılık analizi</p>
        </div>
    </div>
    ...
    <div class="border-b border-gray-200">
        <nav class="flex gap-4 sm:gap-8">
            <button wire:click="switchTab('analyze')" class="py-3 px-1 text-sm font-medium border-b-2 transition-colors whitespace-nowrap {{ $activeTab === 'analyze' ? 'border-purple-600 text-purple-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                🔬 Yeni Analiz
            </button>
            <button wire:click="switchTab('history')" class="py-3 px-1 text-sm font-medium border-b-2 transition-colors whitespace-nowrap {{ $activeTab === 'history' ? 'border-purple-600 text-purple-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                📋 Geçmiş Raporlar
    ...
    @if($activeTab === 'analyze')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
            {{-- Sol: Ürün Veritabanı --}}
            <div class="bg-gradient-to-br from-purple-50 to-fuchsia-100 rounded-xl p-4 lg:p-6 border border-purple-200">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="mt-3 space-y-2">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-800">📦 {{ $this->productCount }} toplam ürün</span>
                            @if($this->costCount > 0)
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-green-100 rounded-xl p-4 lg:p-6 border border-emerald-200">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="font-medium text-gray-900">Flaş Ürünler Excel</h3>
                        <p class="text-sm text-gray-500 mt-1">Trendyol → Flaş Ürünler dosyası</p>
                        <div class="mt-4">
                        ...
                        <div class="mt-3">
                            ...
                            <input type="text" wire:model="reportName" placeholder="Örn: 5 Mart 2026 Flaş Kampanya"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 text-base sm:text-sm min-h-[44px]">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="flex justify-center">
            <button wire:click="analyze" wire:loading.attr="disabled"
                class="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white font-bold rounded-xl hover:from-purple-700 hover:to-fuchsia-700 transition shadow-lg disabled:opacity-50 flex justify-center items-center gap-3 text-lg min-h-[56px]">
        ...
        <div class="flex flex-col items-center justify-center py-12 lg:py-20">
            <div class="w-24 h-24 rounded-full border-4 border-purple-200 border-t-purple-600 animate-spin"></div>
            <h2 class="mt-6 text-xl font-bold text-gray-900">Flaş Analizi Çalışıyor</h2>
            <p class="mt-2 text-sm text-gray-500">24h ve 3h flaş senaryoları hesaplanıyor...</p>
        </div>
        @endif
        ...
        @if($step === 3 && $this->activeReport)
            ...
            {{-- Kampanya Tarihi --}}
            @if(!empty($campaignInfo['campaign_start']) || !empty($campaignInfo['campaign_end']))
            <div class="bg-gradient-to-r from-purple-100 to-fuchsia-100 rounded-lg p-3 flex items-center gap-3 border border-purple-200">
                <span class="font-medium text-purple-800 text-sm">📅 Kampanya:</span>
                <span class="text-purple-600 text-sm">{{ $campaignInfo['campaign_start'] ?? '' }} → {{ $campaignInfo['campaign_end'] ?? '' }}</span>
            </div>
            @endif
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div class="bg-white rounded-xl p-3 border border-green-200">
                    <div class="text-green-500 text-xs">Kârlı Flaş</div>
                ...
                <div class="bg-white rounded-xl p-3 border border-red-200">
                    <div class="text-red-500 text-xs">Zararlı</div>
                    <div class="text-xl font-bold text-red-600">{{ $report->items->where('action', 'warning')->count() }}</div>
                </div>
                <div class="bg-white rounded-xl p-3 border border-gray-200">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 bg-gray-50 rounded-lg p-3">
                <button wire:click="selectAllOpportunities" class="w-full sm:w-auto px-3 py-2 text-sm bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 min-h-[44px]">✓ Kârlıları Seç</button>
                ...
                @if(count($selectedItems) > 0)
                <span class="text-sm font-medium text-purple-600">🛒 {{ count($selectedItems) }} ürün seçildi</span>
                @endif
            </div>

            {{-- AI Panel --}}
            @include('livewire.partials.campaign-ai-panel', ['themeColor' => 'purple'])
            ...
            <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    <table class="w-full text-xs">
                        <thead>
                            <tr class="bg-gray-800 text-white">
                                ...
                                <th class="px-2 py-1.5 text-center bg-purple-600"><span class="text-[10px]">⚡ 24H FLAŞ</span></th>
                                <th class="px-2 py-1.5 text-center bg-fuchsia-600"><span class="text-[10px]">🔥 3H FLAŞ</span></th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            @php
                                ...
                                $flashLabels = ['Mevcut', '24h', '3h'];
                                $flashColors = [
                                    'border-gray-400 bg-gray-50/30',
                                    'border-purple-400 bg-purple-50/30',
                                    'border-fuchsia-400 bg-fuchsia-50/30',
                                ];
                            @endphp
                            <tr wire:key="flash-item-{{ $item->id }}" class="hover:bg-gray-50 {{ $item->action === 'warning' ? 'bg-red-50/50' : '' }}">
                                {{-- Seç --}}
                                <td class="px-1 py-2 text-center align-middle">
                                    <div class="flex flex-col gap-0.5">
                                        @foreach($flashLabels as $idx => $label)
                                            @if(isset($scenarios[$idx]) && ($scenarios[$idx]['price'] ?? 0) > 0)
                                            <button wire:click="selectTariff({{ $item->id }}, {{ $idx }})"
                                                class="px-1.5 py-0.5 rounded text-[8px] font-bold transition-all {{ $selectedIdx === $idx ? $flashColors[$idx] . ' ring-2 text-gray-900 border-2' : 'bg-gray-100 text-gray-400 hover:bg-purple-50 hover:text-purple-600' }}">
                                                {{ $label }}
                                            </button>
                                            @endif
                                        @endforeach
                                ...
                                <td class="px-3 py-2 align-top" style="max-width:220px">
                                    ...
                                    <p class="text-[10px] text-purple-500 font-mono mt-0.5">{{ $item->stock_code }}</p>
                                    @if($item->action === 'update')
                                    <span class="text-[9px] bg-green-100 text-green-700 px-1 py-0.5 rounded font-bold mt-0.5 inline-block">⚡ KARLI</span>
                                ...
                                @php
                                    ...
                                    $cellClass = $isSelected
                                        ? 'border-2 ' . $flashColors[$idx] . ' ring-1 ring-purple-200'
                                        : ($hasTariffSelected ? 'border-gray-100 bg-gray-50/50' : ($isBest ? 'bg-purple-50/80 border border-purple-200' : 'border-gray-100'));
                                @endphp
                                <td class="px-1.5 py-2 text-center align-middle {{ $cellClass }} transition-all cursor-pointer"
                                    ...
                                    <div class="space-y-0.5">
                                        ...
                                        @if($isSelected)
                                        <div class="text-[7px] bg-purple-500 text-white px-1 py-0.5 rounded font-bold inline-block">SEÇİLDİ</div>
                                ...
                                <td class="px-2 py-2 text-center align-middle" x-data="{ editing: false }">
                                    ...
                                    <div class="flex flex-col items-center gap-1 relative">
                                        <div class="relative w-full">
                                            <input type="number" x-ref="fp{{ $item->id }}" value="{{ number_format($dp, 2, '.', '') }}"
                                                @focus="editing = true" @blur="setTimeout(() => editing = false, 200)"
                                                class="w-full text-xs text-center border rounded-md px-1 py-1.5 font-bold focus:ring-2 focus:ring-purple-400 pr-6 {{ $item->custom_price ? 'bg-purple-50 border-purple-300' : 'border-gray-300' }}" step="0.01" min="0">
                                        ...
                                        <button x-show="editing" x-transition x-cloak x-on:click="$wire.updateCustomPrice({{ $item->id }}, $refs.fp{{ $item->id }}.value)"
                                            class="bg-purple-500 hover:bg-purple-600 text-white text-[9px] font-bold px-3 py-1 rounded transition-colors w-full absolute -bottom-6 z-10 shadow-lg">Save</button>
                                        ...
                                        @if(isset($this->suggestedPrices[$item->id]['price']))
                                            <div class="absolute right-full top-0 mr-2 w-52 bg-white border border-purple-200 rounded-lg shadow-2xl z-50 p-2.5 text-left animate-in fade-in slide-in-from-right-2">
                                                <div class="text-[9px] font-bold text-purple-700 mb-1 border-b pb-1">🤖 AI Fiyat Önerisi: {{ number_format($this->suggestedPrices[$item->id]['price'], 2) }} ₺</div>
                                                ...
                                                <div class="flex gap-1 mt-1.5 pt-1.5 border-t">
                                                    <button wire:click.prevent="applySuggestedPrice({{ $item->id }})" class="flex-1 bg-purple-50 text-purple-700 hover:bg-purple-100 text-[8px] py-1 rounded font-bold transition">Uygula</button>
                            ...
                            @empty
                            <tr><td colspan="7" class="px-4 py-8 text-center text-gray-500">Sonuç bulunamadı</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                ...
                <div class="lg:hidden divide-y divide-gray-200">
                    ...
                    @php
                        ...
                        $flashLabels = ['Mevcut', '⚡24h', '🔥3h'];
                    @endphp
                    <div wire:key="flash-mobile-{{ $item->id }}" class="p-4 {{ $item->action === 'warning' ? 'bg-red-50/50' : '' }}">
                        <div class="flex items-start justify-between mb-2">
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-medium text-gray-900 line-clamp-2">{{ $item->product_name ?: $item->stock_code }}</p>
                                <p class="text-[10px] text-purple-500 font-mono">{{ $item->stock_code }}</p>
                            </div>
                            @if($item->action === 'update')
                        ...
                        <div class="grid grid-cols-3 gap-1.5">
                            ...
                            <div wire:click="selectTariff({{ $item->id }}, {{ $idx }})"
                                class="rounded-lg p-2 text-center cursor-pointer transition-all {{ $isSelected ? 'bg-purple-50 border-2 border-purple-400 ring-2 ring-purple-200 shadow-md' : ($isBest && !$hasTariffSelected ? 'bg-purple-50 border-2 border-purple-300' : 'bg-gray-50 border border-gray-200 hover:border-purple-200') }}">
                                <div class="text-[9px] font-bold text-purple-600 mb-0.5">{{ $flashLabels[$idx] ?? '' }}</div>
                                ...
                                @if($isSelected)
                                <div class="text-[7px] bg-purple-500 text-white px-1 py-0.5 rounded font-bold mt-0.5 inline-block">SEÇİLDİ</div>
                                @elseif($isBest && !$hasTariffSelected)
                        ...
                        <div x-data class="mt-3 pt-3 border-t border-gray-100 relative">
                            <div class="flex items-center gap-2">
                                ...
                                <div class="relative flex-1">
                                    <input type="number" x-ref="mfp{{ $item->id }}" value="{{ number_format($dp, 2, '.', '') }}"
                                        class="w-full text-xs text-center border rounded-md px-2 py-1.5 font-bold {{ $item->custom_price ? 'bg-purple-50 border-purple-300' : 'border-gray-300' }} pr-8" step="0.01" min="0">
                                    <button wire:click.prevent="getAiPriceSuggestion({{ $item->id }})" 
                                ...
                                <button x-on:click="$wire.updateCustomPrice({{ $item->id }}, $refs.mfp{{ $item->id }}.value)"
                                    class="bg-purple-500 text-white text-xs font-bold px-4 py-1.5 rounded-md self-stretch hover:bg-purple-600 transition">Kaydet</button>
                            </div>

                            @if(isset($this->suggestedPrices[$item->id]['price']))
                                <div class="mt-2 text-[10px] bg-purple-50 text-purple-800 p-2 rounded-lg border border-purple-100">
                                    ...
                                    <button wire:click.prevent="applySuggestedPrice({{ $item->id }})" class="w-full bg-purple-100 hover:bg-purple-200 text-purple-700 font-bold py-1.5 rounded transition">Uygula</button>
                                </div>
                            @endif
                        </div>
                    </div>
