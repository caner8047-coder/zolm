@php
    ...
    $scenarioColumns = [
        ...
        1 => ['key' => 'flash24', 'label' => '24h Flaş'],
        2 => ['key' => 'flash3', 'label' => '3h Flaş'],
    ];
...
<div class="w-full space-y-4 overflow-x-hidden lg:space-y-6" x-data>
    @once
        <style>
            ...
            .flash-scrollbar::-webkit-scrollbar {
                height: 8px;
                width: 8px;
            }

            .flash-scrollbar::-webkit-scrollbar-thumb {
            ...
            .flash-ledger-table th,
            .flash-ledger-table td {
                vertical-align: top;
            }
            ...
            .flash-resize-handle:hover,
            .flash-resize-handle.active {
                background: #0f172a;
            }
        </style>
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('flashColumnResize', () => ({
                    resizing: false,
                    startX: 0,
                    startWidth: 0,
                    currentTh: null,
    ...
    <section class="rounded-[10px] border border-slate-200 bg-white p-4 shadow-sm lg:p-6">
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-12 xl:gap-6">
            <div class="min-w-0 xl:col-span-7">
                ...
                <h1 class="mt-3 text-xl font-bold text-slate-900 lg:text-2xl">Flaş Ürünler</h1>
                <p class="mt-1 max-w-3xl text-sm text-slate-500">
                    24 saat ve 3 saat flaş fiyatlarını mevcut fiyat, komisyon ve ürün maliyetiyle karşılaştırır; katılım, fiyat ve export kararını tek yüzeyde toplar.
                </p>
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-3 xl:col-span-5">
                ...
                <div class="rounded-[8px] border border-slate-200 bg-slate-50/70 p-3">
                    ...
                    <p class="mt-1 text-xs text-slate-500">Flaş bağlamlı</p>
                </div>
            </div>
        </div>
    </section>
    ...
    @if($activeTab === 'analyze')
        @if($step === 1)
            <div class="grid grid-cols-1 gap-4 lg:gap-6 xl:grid-cols-12">
                <section class="rounded-[10px] border border-slate-200 bg-white p-4 shadow-sm lg:p-6 xl:col-span-8">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div class="min-w-0">
                            <h2 class="text-lg font-semibold text-slate-900">Yeni flaş analizi</h2>
                            <p class="mt-1 text-sm text-slate-500">Flaş Ürünler Excel dosyasını yükleyin; motor mevcut, 24h ve 3h senaryolarını aynı formülle hesaplasın.</p>
                        </div>
                    ...
                    <div class="mt-5 grid grid-cols-1 gap-3 lg:gap-4">
                        <label class="group cursor-pointer rounded-[8px] border border-dashed border-slate-300 bg-slate-50/60 p-4 transition hover:border-slate-400 hover:bg-white">
                            ...
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div class="flex min-w-0 items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-semibold text-slate-900">{{ $excelFile ? $excelFile->getClientOriginalName() : 'Flaş Ürünler dosyası seçin' }}</p>
                                        <p class="mt-1 text-xs text-slate-500">Ürün, fiyat, mevcut komisyon, flaş komisyon ve 24h/3h fiyat kolonları otomatik eşleşir.</p>
                                    </div>
                                </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="Örn: Nisan 2026 Flaş Kampanya Kontrolü"
                    ...
                    @if($excelFile)
                        <div class="mt-5 flex flex-col gap-3 rounded-[8px] border border-emerald-200 bg-emerald-50 p-3 sm:flex-row sm:items-center sm:justify-between">
                            <div class="min-w-0 text-sm text-emerald-800">
                                ...
                                Analiz çalışınca flaş senaryoları kalıcı rapora yazılacak.
                            </div>
                ...
                <aside class="rounded-[10px] border border-slate-200 bg-white p-4 shadow-sm lg:p-6 xl:col-span-4">
                    <h3 class="text-sm font-semibold text-slate-900">Flaş karar akışı</h3>
                    <div class="mt-4 space-y-3">
                        <div class="rounded-[8px] border border-slate-200 bg-slate-50/70 p-3">
                            <p class="text-sm font-semibold text-slate-900">1. Eşleşme</p>
                            <p class="mt-1 text-xs text-slate-500">Barkod, stok kodu, model kodu ve ürün adı ile maliyet kartı bulunur.</p>
                        </div>
                        <div class="rounded-[8px] border border-slate-200 bg-slate-50/70 p-3">
                            <p class="text-sm font-semibold text-slate-900">2. Flaş kıyaslaması</p>
                            <p class="mt-1 text-xs text-slate-500">Mevcut, 24h ve 3h fiyatlar aynı net kâr formülüyle karşılaştırılır.</p>
                        </div>
                        <div class="rounded-[8px] border border-slate-200 bg-slate-50/70 p-3">
                            <p class="text-sm font-semibold text-slate-900">3. AI destekli karar</p>
                            <p class="mt-1 text-xs text-slate-500">AI önerisi, flaş komisyonunu ve satır senaryolarını bağlam olarak kullanır.</p>
                        </div>
                    </div>
                </aside>
            </div>
            ...
            @if($this->costCount === 0)
                <div class="rounded-[10px] border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                    Pazaryeri Ürünlerim modülünde maliyetli ürün bulunamadı. Analiz çalışır, fakat flaş kârlılık kararları güvenilir olmaz.
                </div>
            @endif
        @endif
        ...
        @if($step === 2)
            <section class="rounded-[10px] border border-slate-200 bg-white p-8 text-center shadow-sm lg:p-12">
                ...
                <h2 class="mt-5 text-xl font-semibold text-slate-900">Flaş kampanya motoru çalışıyor</h2>
                <p class="mt-2 text-sm text-slate-500">Ürünler eşleşiyor, maliyetler okunuyor ve 24h/3h fiyatlar net kâra çevriliyor.</p>
            </section>
        @endif
        ...
        @if($step === 3 && $this->activeReport)
            @php
                ...
                $scenarioCounts = $metrics['scenario_counts'];
            @endphp
            ...
            <section class="rounded-[10px] border border-slate-200 bg-white p-4 shadow-sm lg:p-6">
                <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="rounded-[6px] border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-medium text-slate-600">Tamamlanan flaş raporu</span>
                    ...
                    <div class="flex w-full flex-col gap-2 sm:flex-row xl:w-auto">
                        ...
                        <button type="button" wire:click="generateAIAnalysis" x-on:click="$dispatch('openAiPanel', { tab: 'campaign' })" class="inline-flex min-h-[44px] flex-1 items-center justify-center rounded-[6px] border border-violet-200 bg-violet-50 px-4 py-3 text-sm font-medium text-violet-800 transition hover:bg-violet-100 sm:flex-none sm:py-2">
                ...
                <div class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
                    ...
                    <div class="rounded-[8px] border border-slate-200 bg-slate-50/70 p-4">
                        <p class="text-xs font-medium text-slate-500">Flaş kârlı</p>
                        <p class="mt-2 text-2xl font-bold text-violet-700">{{ number_format($report->opportunity_count, 0, ',', '.') }}</p>
                    </div>
                    <div class="rounded-[8px] border border-slate-200 bg-slate-50/70 p-4">
            ...
            <section class="grid grid-cols-1 gap-4 lg:grid-cols-3 lg:gap-6">
                <div class="rounded-[10px] border border-slate-200 bg-white p-4 shadow-sm">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <p class="text-sm font-semibold text-slate-900">Flaş dağılımı</p>
                            <p class="mt-1 text-xs text-slate-500">24h: {{ $scenarioCounts[1] }} · 3h: {{ $scenarioCounts[2] }} · Mevcut: {{ $scenarioCounts[0] }}</p>
                        </div>
                        <span class="rounded-[6px] border border-violet-200 bg-violet-50 px-2 py-1 text-xs font-medium text-violet-800">{{ $report->ai_analysis ? 'Hazır' : 'AI bekliyor' }}</span>
                    </div>
                ...
                <div class="rounded-[10px] border border-slate-200 bg-white p-4 shadow-sm">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <span class="rounded-[6px] border border-violet-200 bg-violet-50 px-2 py-1 text-xs font-medium text-violet-800">{{ $formatMoneyShort($metrics['visible_extra_profit']) }}</span>
                    </div>
                    <div class="mt-4 flex gap-2">
            ...
            <section class="rounded-[10px] border border-slate-200 bg-white shadow-sm" x-data="{ columnsOpen: false }">
                <div class="border-b border-slate-200 p-4 lg:p-5">
                    <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                        <div class="min-w-0">
                            <h3 class="text-lg font-semibold text-slate-900">Flaş karar ledger'i</h3>
                        ...
                        <div class="flex w-full flex-col gap-2 sm:flex-row xl:w-auto">
                            <button type="button" wire:click="selectAllOpportunities" class="inline-flex min-h-[44px] flex-1 items-center justify-center rounded-[6px] border border-violet-200 bg-violet-50 px-4 py-3 text-sm font-medium text-violet-800 transition hover:bg-violet-100 sm:flex-none sm:py-2">
                                Tüm kârlıları seç
                            </button>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-3 xl:grid-cols-12">
                        <div class="xl:col-span-4">
                            <label class="sr-only" for="flash-search">Ürün ara</label>
                            <input
                                id="flash-search"
                                type="search"
                ...
                <div class="hidden md:block">
                    <div class="flash-scrollbar overflow-x-auto" x-data="flashColumnResize()">
                        <table class="flash-ledger-table w-full text-xs">
                            <thead class="border-b border-slate-200 bg-slate-50 text-slate-600">
                                <tr>
                                    ...
                                    <th class="relative w-[300px] px-3 py-3 text-left font-semibold">
                                        ...
                                        <span class="flash-resize-handle" x-on:mousedown.prevent="startResize($event, $el.closest('th'))"></span>
                                    </th>
                                    ...
                                    @foreach($scenarioColumns as $idx => $column)
                                        @if($this->showColumn($column['key']))
                                            <th class="relative w-40 px-3 py-3 text-center font-semibold">
                                                {{ $column['label'] }}
                                                <span class="flash-resize-handle" x-on:mousedown.prevent="startResize($event, $el.closest('th'))"></span>
                                            </th>
                                        @endif
                            ...
                            <tbody class="divide-y divide-slate-100 bg-white">
                                @forelse($items as $item)
                                    @php
                                        ...
                                        $statusLabel = $item->action === 'update' ? 'Flaş kârlı' : ($item->action === 'warning' ? 'Risk' : 'Koru');
                                    ...
                                    @endphp
                                    <tr wire:key="flash-row-{{ $item->id }}" class="transition hover:bg-slate-50/80">
                                        <td class="px-3 py-3">
                                            ...
                                            @if($this->showColumn($column['key']))
                                                @php
                                                    ...
                                                    $scenarioShell = $isSelected
                                                        ...
                                                        : ($isBest ? ($idx > 0 ? 'border-violet-200 bg-violet-50 text-slate-900' : 'border-slate-200 bg-slate-50 text-slate-900') : 'border-slate-200 bg-white text-slate-900 hover:border-slate-300 hover:bg-slate-50');
                                                @endphp
                                                    ...
                                                    @if($scenario && ($idx === 0 || (float) ($scenario['price'] ?? 0) > 0))
                                                        <button type="button" wire:click="selectTariff({{ $item->id }}, {{ $idx }})" class="w-full rounded-[8px] border p-2 text-left transition {{ $scenarioShell }}">
                                                            <div class="flex items-center justify-between gap-2">
                                                                ...
                                                                @elseif($isBest)
                                                                    <span class="rounded-[6px] {{ $idx > 0 ? 'bg-violet-700' : 'bg-slate-700' }} px-1.5 py-0.5 text-[9px] font-semibold text-white">En iyi</span>
                                                                @endif
                                                            </div>
                                            ...
                                            <td class="px-3 py-3">
                                                <div class="space-y-2" x-data>
                                                    <div class="flex items-center gap-2">
                                                        ...
                                                        <button type="button" wire:click="getAiPriceSuggestion({{ $item->id }})" class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[6px] border border-violet-200 bg-violet-50 text-xs font-semibold text-violet-800 transition hover:bg-violet-100" title="AI fiyat öner">
                                                            <span wire:loading.remove wire:target="getAiPriceSuggestion({{ $item->id }})">AI</span>
                                                            <span wire:loading wire:target="getAiPriceSuggestion({{ $item->id }})" class="h-4 w-4 animate-spin rounded-full border-2 border-violet-200 border-t-violet-700"></span>
                                                        </button>
                                                    </div>
                                                    ...
                                                    @if($suggestion && !($suggestion['loading'] ?? false))
                                                        <div class="rounded-[6px] border {{ isset($suggestion['error']) ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-violet-200 bg-violet-50 text-violet-900' }} p-2 text-[11px]">
                                                            @if(isset($suggestion['error']))
                                                                {{ $suggestion['error'] }}
                                                            @else
                                                                <div class="flex items-center justify-between gap-2">
                                                                    <span class="font-semibold">{{ $formatMoney($suggestion['price']) }}</span>
                                                                    <button type="button" wire:click="applySuggestedPrice({{ $item->id }})" class="rounded-[6px] bg-violet-700 px-2 py-1 text-[10px] font-semibold text-white">Uygula</button>
                                                                </div>
                                                                <p class="mt-1 text-violet-800">{{ \Illuminate\Support\Str::limit($suggestion['reason'] ?? '', 72) }}</p>
                                                            @endif
                                                        </div>
                                                    @endif
                                                </div>
                ...
                <div class="divide-y divide-slate-200 md:hidden">
                    @forelse($items as $item)
                        @php
                            ...
                            $isChecked = in_array($item->id, $selectedItems, true);
                        @endphp
                        <article wire:key="flash-mobile-{{ $item->id }}" class="p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                            ...
                            <div class="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                                @foreach($scenarioColumns as $idx => $column)
                                    @php
                                        ...
                                        $scenarioShell = $isSelected
                                            ? 'border-slate-900 bg-slate-900 text-white'
                                            : ($isBest ? ($idx > 0 ? 'border-violet-200 bg-violet-50' : 'border-slate-200 bg-slate-50') : 'border-slate-200 bg-white');
                                    @endphp
                                    @if($scenario && ($idx === 0 || (float) ($scenario['price'] ?? 0) > 0))
                                        <button type="button" wire:click="selectTariff({{ $item->id }}, {{ $idx }})" class="rounded-[8px] border p-3 text-left {{ $scenarioShell }}">
                                            <div class="flex items-center justify-between gap-2">
                                                ...
                                                @elseif($isBest)
                                                    <span class="rounded-[6px] {{ $idx > 0 ? 'bg-violet-700' : 'bg-slate-700' }} px-1.5 py-0.5 text-[10px] text-white">En iyi</span>
                                                @endif
                                            </div>
                            ...
                            <div class="mt-3 rounded-[8px] border border-slate-200 bg-slate-50/70 p-3" x-data>
                                ...
                                <div class="mt-2 flex items-center gap-2">
                                    ...
                                    <button type="button" wire:click="getAiPriceSuggestion({{ $item->id }})" class="min-h-[44px] rounded-[6px] border border-violet-200 bg-violet-50 px-3 text-sm font-semibold text-violet-800">AI</button>
                                </div>
                                ...
                                @if($suggestion && !($suggestion['loading'] ?? false) && !isset($suggestion['error']))
                                    <div class="mt-2 rounded-[6px] border border-violet-200 bg-violet-50 p-2 text-xs text-violet-900">
                                        <div class="flex items-center justify-between gap-2">
                                            <span class="font-semibold">{{ $formatMoney($suggestion['price']) }}</span>
                                            <button type="button" wire:click="applySuggestedPrice({{ $item->id }})" class="rounded-[6px] bg-violet-700 px-2 py-1 text-[11px] font-semibold text-white">Uygula</button>
                                        </div>
    ...
    @if($activeTab === 'history')
        <section class="rounded-[10px] border border-slate-200 bg-white p-4 shadow-sm lg:p-6">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-lg font-semibold text-slate-900">Geçmiş flaş raporları</h2>
            ...
            @if($this->reports->isEmpty())
                <div class="mt-5 rounded-[8px] border border-dashed border-slate-300 bg-slate-50/60 p-8 text-center">
                    ...
                    <p class="mt-1 text-sm text-slate-500">İlk flaş dosyasını analiz ettiğinizde raporlar burada listelenir.</p>
                </div>
            @else
                        ...
                        <article class="rounded-[8px] border border-slate-200 bg-slate-50/60 p-4 transition hover:bg-white hover:shadow-sm">
                            ...
                            <div class="mt-4 grid grid-cols-3 gap-2">
                                ...
                                <div class="rounded-[6px] border border-slate-200 bg-white p-2">
                                    ...
                                    <p class="mt-1 font-semibold text-violet-700">{{ number_format($rpt->opportunity_count, 0, ',', '.') }}</p>
                                </div>
                                <div class="rounded-[6px] border border-slate-200 bg-white p-2">
    ...
    @include('livewire.partials.campaign-ai-panel', ['themeColor' => 'violet'])
</div>
