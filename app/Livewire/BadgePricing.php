...
class BadgePricing extends Component
{
    use WithFileUploads;
    ...
    public array $sortableColumns = [
        ...
        'suggested_net_profit' => 'Yıldız kârı',
    ];
    ...
    public function filteredItems(): Collection
    {
        ...
        $items = $items->filter(function ($item) {
            return match ($this->statusFilter) {
                ...
                'star1', 'star2', 'star3' => $this->bestScenarioIndex($item) === (int) Str::after($this->statusFilter, 'star'),
                default => true,
            };
        });
    ...
    public function reportMetrics(): array
    {
        ...
        foreach ($items as $item) {
            $starCounts[$this->bestScenarioIndex($item)]++;
        }
    ...
    public function analyze()
    {
        ...
        $service = app(BadgePricingService::class);
    ...
    public function updateCustomPrice(int $itemId, $newPrice)
    {
        ...
        // Hangi yıldız aralığına düşüyor?
        $scenarios = $item->scenario_details;
        foreach ([3, 2, 1] as $idx) {
            if (isset($scenarios[$idx]) && $newPrice <= $scenarios[$idx]['price'] && $scenarios[$idx]['price'] > 0) {
                $commission = $scenarios[$idx]['commission'];
                break;
            }
        }
    ...
    public function selectFilteredOpportunities()
    {
        ...
        if (empty($this->selectedItems)) {
            $this->message = 'Görünen ürünler içinde seçilecek yıldız fırsatı bulunamadı.';
    ...
    {
        if (!in_array($filter, ['all', 'opportunity', 'risk', 'selected', 'missing_cost', 'unmatched', 'kept', 'star1', 'star2', 'star3'], true)) {
            return;
        }
    ...
    public function exportSelected()
    {
        ...
        if (empty($exportIds)) {
            $this->message = 'Lütfen önce en az bir ürün için yıldız seçin.';
            $this->messageType = 'error';
            return;
        }
    ...
    public function generateAIAnalysis()
    {
        ...
        if (!$report) return;
        $this->isProcessing = true;
        $this->message = 'Yıldız etiketi analizi yapılıyor...';
        try {
            $service = app(\App\Services\AIService::class);
            $analysis = $service->analyzeBadgeCampaign($report);
        ...
        if ($this->showChat && !$this->chatConversationId && $this->activeReportId) {
            ...
            if (!$conversation) {
                $conversation = AIConversation::create([
                    ...
                    'messages' => [['role' => 'system', 'content' => 'Merhaba! Yıldız etiketi raporunuz hakkında sorularınızı yanıtlayabilirim.', 'timestamp' => now()->toISOString()]]
                ]);
            }
    ...
    {
        return view('livewire.badge-pricing')
            ->layout('layouts.app', ['title' => 'Avantajlı Ürün Etiketleri']);
    }
}
