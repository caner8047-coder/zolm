...
class PlusCommission extends Component
{
    use WithFileUploads;
    ...
    public function analyze()
    {
        ...
        $service = app(PlusCommissionService::class);
    ...
    public function updateCustomPrice(int $itemId, $newPrice)
    {
        ...
        // Plus komisyon oranını kullan (senaryo 1)
        $plusScenario = $item->scenario_details[1] ?? null;
        $commission = $plusScenario ? $plusScenario['commission'] : $item->current_commission;
    ...
    public function generateAIAnalysis()
    {
        ...
        if (!$report) return;
        $this->isProcessing = true;
        $this->message = 'Plus kampanya analizi yapılıyor...';
        try {
            $service = app(\App\Services\AIService::class);
            $analysis = $service->analyzePlusCampaign($report);
        ...
        if ($this->showChat && !$this->chatConversationId && $this->activeReportId) {
            ...
            if (!$conversation) {
                $conversation = AIConversation::create([
                    ...
                    'messages' => [['role' => 'system', 'content' => 'Merhaba! Plus kampanya raporunuz hakkında sorularınızı yanıtlayabilirim.', 'timestamp' => now()->toISOString()]]
                ]);
            }
    ...
    {
        return view('livewire.plus-commission')
            ->layout('layouts.app', ['title' => 'Plus Komisyon Tarifeleri']);
    }
}
