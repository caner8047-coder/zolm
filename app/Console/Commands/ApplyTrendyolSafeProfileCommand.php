...
class ApplyTrendyolSafeProfileCommand extends Command
{
    protected $signature = 'marketplace:apply-trendyol-safe-profile
        {--store= : Sadece belirli Trendyol mağaza ID}
        {--all : Tüm Trendyol mağazalarına uygula}
        {--dry-run : Yalnızca etkilenecek mağazaları göster, kayıt yapma}';

    protected $description = 'Trendyol mağazalarına düşük etkili ve güvenli sync profilini uygular.';
    ...
    public function handle(): int
    {
        ...
        if (!$applyAll && !$storeId) {
            $this->error('Bir Trendyol mağazası seçmek için --store=ID kullanın veya tümü için --all verin.');
        ...
        if ($stores->isEmpty()) {
            $this->warn('Uygulanacak Trendyol mağazası bulunamadı.');
        ...
        if ($dryRun) {
            $this->newLine();
            $this->components->warn("Dry-run tamamlandı. {$stores->count()} Trendyol mağazası güvenli profil için uygun.");
        ...
        $this->newLine();
        $this->components->info("Trendyol güvenli profili {$stores->count()} mağazaya uygulandı.");
