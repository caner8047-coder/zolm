...
class ApplyWooCommerceSafeProfileCommand extends Command
{
    protected $signature = 'marketplace:apply-woo-safe-profile
        {--store= : Sadece belirli WooCommerce mağaza ID}
        {--all : Tüm WooCommerce mağazalarına uygula}
        {--dry-run : Yalnızca etkilenecek mağazaları göster, kayıt yapma}';

    protected $description = 'WooCommerce mağazalarına düşük etkili ve güvenli sync profilini uygular.';
    ...
    public function handle(): int
    {
        ...
        if (!$applyAll && !$storeId) {
            $this->error('Bir Woo mağazası seçmek için --store=ID kullanın veya tümü için --all verin.');
        ...
        if ($stores->isEmpty()) {
            $this->warn('Uygulanacak WooCommerce mağazası bulunamadı.');
        ...
        if ($dryRun) {
            $this->newLine();
            $this->components->warn("Dry-run tamamlandı. {$stores->count()} WooCommerce mağazası güvenli profil için uygun.");
        ...
        $this->newLine();
        $this->components->info("WooCommerce güvenli profili {$stores->count()} mağazaya uygulandı.");
