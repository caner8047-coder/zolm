...
class ApplyHepsiburadaSafeProfileCommand extends Command
{
    protected $signature = 'marketplace:apply-hepsiburada-safe-profile
        {--store= : Sadece belirli Hepsiburada mağaza ID}
        {--all : Tüm Hepsiburada mağazalarına uygula}
        {--dry-run : Yalnızca etkilenecek mağazaları göster, kayıt yapma}';

    protected $description = 'Hepsiburada mağazalarına düşük etkili ve güvenli sync profilini uygular.';
    ...
    public function handle(): int
    {
        ...
        if (!$applyAll && !$storeId) {
            $this->error('Bir Hepsiburada mağazası seçmek için --store=ID kullanın veya tümü için --all verin.');
        ...
        if ($stores->isEmpty()) {
            $this->warn('Uygulanacak Hepsiburada mağazası bulunamadı.');
        ...
        $this->table(
            ['Store ID', 'Mağaza', 'Sipariş', 'Finans', 'Ürün', 'Push', 'Webhook', 'Paralel', 'Jitter'],
            $stores->map(function (MarketplaceStore $store) use ($defaults): array {
                return [
                    ...
                    $defaults['webhook_enabled'] ? 'Açık' : 'Kapalı',
        ...
        if ($dryRun) {
            $this->newLine();
            $this->components->warn("Dry-run tamamlandı. {$stores->count()} Hepsiburada mağazası güvenli profil için uygun.");
        ...
        $this->newLine();
        $this->components->info("Hepsiburada güvenli profili {$stores->count()} mağazaya uygulandı.");
