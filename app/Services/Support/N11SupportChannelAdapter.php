...
class N11SupportChannelAdapter implements SupportChannelAdapterInterface
{
    public function key(): string { return 'n11'; }
    public function name(): string { return 'N11'; }
    ...
    public function getCapabilities(?SupportChannel $channel = null): array
    {
        ...
        if ($channel) {
            ...
            $isN11 = $store && $store->marketplace === 'n11';

            if ($hasConnection && $isEnabled && $isN11) {
    ...
    {
        $store = $channel->store;
        if (!$store || $store->marketplace !== 'n11') {
            return ['status' => 'error', 'message' => 'N11 mağazası bulunamadı'];
        }

        $connection = $store->connection;
        if (!$connection || $connection->status !== 'configured') {
            return ['status' => 'not_configured', 'message' => 'N11 bağlantısı tanımlı değil'];
        }

        return ['status' => 'ok', 'message' => 'N11 bağlantısı aktif'];
    }

    public function syncConversations(SupportChannel $channel): array
    {
        return ['synced' => 0, 'message' => 'N11 sync mevcut connector üzerinden yönetilir'];
    }
    ...
    {
        // Sıkı biçim kontrolü
        if (!preg_match('/^n11_questions_(\d+)$/', $conversationExternalId, $matches)) {
        ...
        $lockKey = null;
        if ($idempotencyKey) {
            $lockKey = "idemp_n11_reply_" . md5($idempotencyKey);
        ...
        try {
            ...
            if ($log->status === 'sent') {
                $channelMsgId = (string)($log->external_answer_id ?? uniqid('n11_'));
                if ($lockKey) {
